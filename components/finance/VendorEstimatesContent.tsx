...
import { getVendorCategories } from "@/app/actions/vendor-finance";
import { 
    getVendorEstimates, createVendorEstimate, updateVendorEstimateStatus, deleteVendorEstimate, 
    type EstimateType 
...
// ─── Edit Vendor Estimate Modal ──────────────────────────────────────────────
function EditVendorEstimateModal({
    estimate,
    categories,
    onClose,
    onSave,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const { updateVendorEstimate } = await import("@/app/actions/finance-estimates");
            const res = await updateVendorEstimate(estimate.id, {
            ...
            if (res.success) {
                toast.success("Budget updated and resubmitted.");
                onSave();
                onClose();
            } else {
...
// ─── Propose Vendor Estimate Modal ──────────────────────────────────────────

function ProposeVendorEstimateModal({
    categories,
    onClose,
    onSave,
}: {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const res = await createVendorEstimate({
                monthYYYYMM,
...
export default function VendorEstimatesContent() {
    const toast = useToast();
    ...
    const approveEstimate = async (id: number) => {
        ...
        setPendingAction(id);
        const res = await updateVendorEstimateStatus(id, "Approved");
        if (res.success) {
    ...
    const rejectEstimate = async (id: number) => {
        ...
        setPendingAction(id);
        const res = await updateVendorEstimateStatus(id, "Rejected");
        if (res.success) {
    ...
    const handleDelete = async (id: number) => {
        ...
        setPendingAction(id);
        const res = await deleteVendorEstimate(id);
        if (res.success) {
