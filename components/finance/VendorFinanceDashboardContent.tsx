...
import { getVendorFinanceDashboard, getVendorFinanceYears } from "@/app/actions/vendor-finance";
import type { VendorFinanceDashboard } from "@/app/actions/vendor-finance";
import {
...
export default function VendorFinanceDashboardContent() {
    const [year, setYear] = useState(CURRENT_YEAR);
    const [years, setYears] = useState<number[]>([CURRENT_YEAR]);
    const [data, setData] = useState<VendorFinanceDashboard | null>(null);
    const [loading, setLoading] = useState(true);

    const load = useCallback(async () => {
        setLoading(true);
        const [dashRes, yearRes] = await Promise.all([
            getVendorFinanceDashboard(year),
            getVendorFinanceYears()
        ]);
        if (dashRes.success && dashRes.data) setData(dashRes.data as VendorFinanceDashboard);
