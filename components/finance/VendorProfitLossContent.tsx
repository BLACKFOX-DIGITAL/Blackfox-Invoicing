...
import { getVendorProfitLossReport, getVendorFinanceYears, getVendorYearlyComparisonReport } from "@/app/actions/vendor-finance";
import type { VendorPLReport, VendorYearlyComparisonReport } from "@/app/actions/vendor-finance";
...
export default function VendorProfitLossContent() {
    const [viewMode, setViewMode] = useState<"monthly" | "yearly">("monthly");
    const [year, setYear] = useState(CURRENT_YEAR);
    const [years, setYears] = useState<number[]>([CURRENT_YEAR]);
    const [monthlyReport, setMonthlyReport] = useState<VendorPLReport | null>(null);
    const [yearlyReport, setYearlyReport] = useState<VendorYearlyComparisonReport | null>(null);
    ...
    const loadData = useCallback(async () => {
        ...
        if (viewMode === "monthly") {
            const [repRes, yearRes] = await Promise.all([
                getVendorProfitLossReport(year),
                getVendorFinanceYears()
            ]);
            if (repRes.success && repRes.data) setMonthlyReport(repRes.data as VendorPLReport);
            if (yearRes.success && yearRes.data) setYears(yearRes.data as number[]);
        } else {
            const res = await getVendorYearlyComparisonReport();
            if (res.success && res.data) setYearlyReport(res.data as VendorYearlyComparisonReport);
        }
        setLoading(false);
    }, [year, viewMode]);
