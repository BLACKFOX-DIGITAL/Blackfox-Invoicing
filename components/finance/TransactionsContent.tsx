...
import {
    getTransactions, getCategories, createTransaction,
    updateTransaction, deleteTransaction, getFinanceYears,
    getFinanceEmployees, recordLoanDisbursement, recordLoanRepayment
} from "@/app/actions/finance";
import type { FinanceTransactionWithCategory, FinanceCategoryWithCount } from "@/app/actions/finance";
import {
    Plus, Pencil, Trash2, Download, Search, Filter,
    X, Check, Loader2, AlertCircle, Receipt, TrendingUp, TrendingDown,
...
function EditTransactionModal({
    ...
    transaction: FinanceTransactionWithCategory;
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="e.g. Employee Salary - March"
                            className="w-full px-3 py-2 bg-bg-app border border-border-subtle rounded-lg text-sm text-text-main focus:outline-none focus:border-primary transition-all"
                        />
                    </div>
...
function DeleteTxModal({
    ...
    tx: FinanceTransactionWithCategory;
    onClose: () => void;
    onDeleted: () => void;
}) {
    const toast = useToast();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-bg-sidebar rounded-2xl border border-border-subtle w-full max-w-sm shadow-2xl p-6 space-y-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-base font-bold text-text-main">Delete Transaction</h2>
                        <p className="text-sm text-text-muted truncate max-w-[200px]">{tx.description}</p>
                    </div>
                </div>
...
export default function TransactionsContent() {
    const [transactions, setTransactions] = useState<FinanceTransactionWithCategory[]>([]);
    ...
    const [editTarget, setEditTarget] = useState<FinanceTransactionWithCategory | null>(null);
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [isLoanModalOpen, setIsLoanModalOpen] = useState(false);
    const [deleteTarget, setDeleteTarget] = useState<FinanceTransactionWithCategory | null>(null);
    ...
    const loadData = useCallback(async () => {
        ...
        if (txRes.success && txRes.data) setTransactions(txRes.data as FinanceTransactionWithCategory[]);
    ...
    // Running balance
    const withBalance = transactions.reduce<(FinanceTransactionWithCategory & { balance: number })[]>(
        (acc, tx) => {
    ...
    const handleExportCSV = () => {
        ...
        transactions.forEach((t) => {
            ...
            rows.push(`${d},"${t.category.name}",${t.category.type},"${t.description}",${t.amount.toFixed(2)},"${t.notes || ""}"`);
        });
        const blob = new Blob([rows.join("\n")], { type: "text/csv" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `finance_transactions_${year}${month ? "_" + MONTHS[month - 1] : ""}.csv`;
        a.click();
        ...
        <>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-text-main flex items-center gap-2">
                        <Receipt size={22} className="text-primary" /> Transactions
                    </h1>
                    <p className="text-text-muted text-sm mt-1">All BDT income and expense records</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <Card className="p-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xs text-text-muted uppercase tracking-wider font-medium">Total Income</p>
                            <p className="text-xl font-bold text-status-success">{BDT(totalIncome)}</p>
                        </div>
                    </div>
                </Card>
                <Card className="p-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-bold text-status-error">{BDT(totalExpenses)}</p>
                        </div>
                    </div>
                </Card>
                <Card className="p-4">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-xl ${netProfit >= 0 ? "bg-primary/10" : "bg-status-error/10"}`}>
                            <Receipt size={18} className={netProfit >= 0 ? "text-primary" : "text-status-error"} />
                        </div>
                        <div>
                            <p className="text-xs text-text-muted uppercase tracking-wider font-medium">Net Profit / Loss</p>
                            <p className={`text-xl font-bold ${netProfit >= 0 ? "text-primary" : "text-status-error"}`}>
                            ...
                            <thead>
                                <tr className="border-b border-border-subtle bg-bg-app/50">
                                    <th className="px-4 py-3">
                                        <SortableHeader label="Date" sortKey="date" currentSort={sortField} currentOrder={sortOrder} onSort={handleSort} className="text-text-muted justify-start" />
                                    </th>
                                    <th className="px-4 py-3">
                                        <SortableHeader label="Category" sortKey="category" currentSort={sortField} currentOrder={sortOrder} onSort={handleSort} className="text-text-muted justify-start" />
                                    </th>
                                    <th className="px-4 py-3">
                                        <SortableHeader label="Description" sortKey="description" currentSort={sortField} currentOrder={sortOrder} onSort={handleSort} className="text-text-muted justify-start" />
                                    </th>
                                    <th className="px-4 py-3 text-right">
                                        <SortableHeader label="Amount" sortKey="amount" currentSort={sortField} currentOrder={sortOrder} onSort={handleSort} className="text-text-muted justify-end pr-0" />
                                    </th>
                                    <th className="px-4 py-3 text-right">
                                        <SortableHeader label="Balance" sortKey="balance" currentSort={sortField} currentOrder={sortOrder} onSort={handleSort} className="text-text-muted justify-end pr-0" />
                                    </th>
                                ...
                                {sortedData.map((tx) => (
                                    <tr key={tx.id} className="hover:bg-bg-app/40 transition-colors group">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold ${tx.category.type === "Revenue"
                                                ? "bg-status-success/10 text-status-success"
                                                : "bg-status-error/10 text-status-error"
                                                }`}>
                                                {tx.category.type === "Revenue" ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
                                                {tx.category.name}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                                                <button onClick={() => setEditTarget(tx)}
                                                    className="p-1.5 text-text-muted hover:text-primary rounded-lg hover:bg-primary/10 transition-all">
                                                    <Pencil size={14} />
                                                </button>
                                                <button onClick={() => setDeleteTarget(tx)}
                                                    className="p-1.5 text-text-muted hover:text-status-error rounded-lg hover:bg-status-error/10 transition-all">
                                                    <Trash2 size={14} />
                                                </button>
                                            </div>
                                        </td>
            ...
            {isLoanModalOpen && (
                <LoanManagementModal
                    ...
                    onSave={loadData}
                    getEmployees={getFinanceEmployees}
                    recordDisbursement={recordLoanDisbursement}
