...
import {
    getVendorCategories, createVendorCategory,
    updateVendorCategory, deleteVendorCategory
} from "@/app/actions/vendor-finance";
import type { VendorFinanceCategoryWithCount } from "@/app/actions/vendor-finance";
import {
...
function CategoryModal({ category, onClose, onSave }: {
    category: VendorFinanceCategoryWithCount | null;
    onClose: () => void;
    onSave: () => void;
}) {
    ...
    const [type, setType] = useState<"Revenue" | "Expense">((category?.type as any) || "Expense");
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const res = category
                ? await updateVendorCategory(category.id, { name: name.trim(), type, description: description.trim() || undefined })
                : await createVendorCategory({ name: name.trim(), type, description: description.trim() || undefined });
            if (res.success) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-bg-sidebar rounded-2xl border border-border-subtle w-full max-w-md shadow-2xl">
                <div className="flex items-center justify-between p-6 border-b border-border-subtle">
                    <h2 className="text-lg font-bold text-text-main">{category ? "Edit Category" : "New Category"}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider block mb-1.5">Type <span className="text-status-error">*</span></label>
                            ...
                            {(["Revenue", "Expense"] as const).map((t) => (
                                <button key={t} type="button" onClick={() => setType(t)}
                                    className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-semibold transition-all ${type === t
                                        ? t === "Revenue" ? "bg-status-success/15 border-status-success text-status-success" : "bg-status-error/15 border-status-error text-status-error"
                                        : "border-border-subtle text-text-muted hover:bg-bg-app"}`}>
                                    {t === "Revenue" ? <TrendingUp size={14} /> : <TrendingDown size={14} />}{t}
                                </button>
                            ))}
                        </div>
                    </div>
                    <div>
                        <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider block mb-1.5">Name <span className="text-status-error">*</span></label>
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Staff Payment"
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider block mb-1.5">Description</label>
...
function DeleteModal({ category, onClose, onDeleted }: {
    category: VendorFinanceCategoryWithCount;
    onClose: () => void;
    onDeleted: () => void;
}) {
    const toast = useToast();
    ...
    const handleDelete = () => {
        startTransition(async () => {
            const res = await deleteVendorCategory(category.id);
            if (res.success) { toast.success("Category deleted."); onDeleted(); onClose(); }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-bg-sidebar rounded-2xl border border-border-subtle w-full max-w-sm shadow-2xl p-6 space-y-4">
                <div className="flex items-center gap-3">
                    ...
                    <div><h2 className="text-lg font-bold text-text-main">Delete Category</h2><p className="text-sm text-text-muted">{category.name}</p></div>
                </div>
                ...
                {!error && <p className="text-sm text-text-muted">Are you sure? This cannot be undone. Categories with existing transactions cannot be deleted.</p>}
...
export default function VendorCategoriesContent() {
    ...
    const [editTarget, setEditTarget] = useState<VendorFinanceCategoryWithCount | null | "new">(null);
    const [deleteTarget, setDeleteTarget] = useState<VendorFinanceCategoryWithCount | null>(null);

    const loadCategories = useCallback(async () => {
        setLoading(true);
        const res = await getVendorCategories();
        if (res.success && res.data) setCategories(res.data as VendorFinanceCategoryWithCount[]);
        setLoading(false);
    }, []);
    ...
    return (
        <>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-text-main flex items-center gap-2"><Tag size={22} className="text-primary" /> Finance Categories</h1>
                    <p className="text-text-muted text-sm mt-1">Manage Frame IT income and expense categories</p>
                </div>
                <Button onClick={() => setEditTarget("new")} className="flex items-center gap-2"><Plus size={16} /> Add Category</Button>
            </div>

            {loading ? (
                <div className="flex items-center justify-center h-48 text-text-muted"><Loader2 size={28} className="animate-spin" /></div>
            ) : categories.length === 0 ? (
                <Card className="p-12 text-center">
                    ...
                    <p className="text-text-muted max-w-sm mx-auto mb-4">Create categories to classify your income and expenses.</p>
                    <Button onClick={() => setEditTarget("new")} className="inline-flex items-center gap-2"><Plus size={16} /> Add First Category</Button>
                </Card>
            ) : (
                            ...
                            {revenue.length === 0 ? (
                                ...
                                <div key={cat.id} className="flex items-center gap-3 px-5 py-3 hover:bg-bg-app transition-colors group">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {cat.description && <p className="text-xs text-text-muted truncate">{cat.description}</p>}
                                    </div>
                                    <span className="text-xs text-text-muted shrink-0">{cat._count.transactions} txn{cat._count.transactions !== 1 ? "s" : ""}</span>
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => setEditTarget(cat)} className="p-1.5 text-text-muted hover:text-primary rounded-lg hover:bg-primary/10 transition-all"><Pencil size={14} /></button>
                                        <button onClick={() => setDeleteTarget(cat)} className="p-1.5 text-text-muted hover:text-status-error rounded-lg hover:bg-status-error/10 transition-all"><Trash2 size={14} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            {editTarget !== null && (
                <CategoryModal category={editTarget === "new" ? null : editTarget} onClose={() => setEditTarget(null)} onSave={loadCategories} />
            )}
            {deleteTarget && (
                <DeleteModal category={deleteTarget} onClose={() => setDeleteTarget(null)} onDeleted={loadCategories} />
            )}
        </>
    );
}
